...
interface TaskStats {
  subscription: { today_count: number; total_count: number }
  comment: { today_count: number; total_count: number }
  view: { today_count: number; total_count: number }
}
...
export default function Profile() {
    ...
    if (user) {
      ...
      if (!defaultCountry && initData?.user) {
        ...
        const languageToCountry: Record<string, string> = {
          'RU': 'Россия', 'EN': 'США', 'UK': 'Украина', 'BY': 'Беларусь', 'KZ': 'Казахстан',
          'UZ': 'Узбекистан', 'AZ': 'Азербайджан', 'AM': 'Армения', 'GE': 'Грузия',
          'MD': 'Молдова', 'KG': 'Кыргызстан', 'TJ': 'Таджикистан', 'TM': 'Туркменистан',
          'DE': 'Германия', 'FR': 'Франция', 'ES': 'Испания', 'IT': 'Италия', 'PL': 'Польша',
          'TR': 'Турция', 'IN': 'Индия', 'ID': 'Индонезия', 'BR': 'Бразилия', 'MX': 'Мексика',
          'AR': 'Аргентина', 'CN': 'Китай', 'JP': 'Япония', 'KR': 'Корея'
        }
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      await updateUser({ age: Number(age), gender, country, terms_accepted: true })
    ...
    <div className="profile-page">
      ...
      {isBanned && (
        <div className="ban-notice" style={{
          background: '#ffebee', border: '2px solid #f44336', borderRadius: '8px',
          padding: '20px', marginBottom: '20px', color: '#c62828'
        }}>
          ...
          <div style={{ background: 'white', padding: '15px', borderRadius: '6px', marginTop: '15px', border: '1px solid #ffcdd2' }}>
      ...
      <div className="profile-card">
        ...
        {!profileFilled ? (
          <div className="profile-form">
            ...
            <div className="terms-section">
              <label className="terms-checkbox-label">
                ...
                <span>
                  ...
                  <button type="button" className="terms-link" onClick={() => setShowAgreementModal(true)}>
            ...
            <button className="save-button" onClick={handleSave} disabled={saving || !isComplete || !termsAccepted}>
      ...
      {taskStats && (
        <div className="profile-task-stats">
          <div className="profile-task-stat profile-task-stat-view">
            <div className="stat-title">Просмотр</div>
            <div className="stat-row"><span>сегодня</span><strong>{taskStats.view.today_count}</strong></div>
            <div className="stat-row"><span>всего</span><strong>{taskStats.view.total_count}</strong></div>
          </div>
          <div className="profile-task-stat profile-task-stat-subscription">
            <div className="stat-title">Подписка</div>
            <div className="stat-row"><span>сегодня</span><strong>{taskStats.subscription.today_count}</strong></div>
            <div className="stat-row"><span>всего</span><strong>{taskStats.subscription.total_count}</strong></div>
          </div>
          <div className="profile-task-stat profile-task-stat-comment">
            <div className="stat-title">Комментарий</div>
            <div className="stat-row"><span>сегодня</span><strong>{taskStats.comment.today_count}</strong></div>
            <div className="stat-row"><span>всего</span><strong>{taskStats.comment.total_count}</strong></div>
          </div>
        </div>
      )}

      {showTermsModal && (
        <TermsModal title="Правила пользования приложением" content={TERMS_TEXT} onClose={() => setShowTermsModal(false)} />
      )}
      {showAgreementModal && (
        <TermsModal title="Пользовательское соглашение" content={AGREEMENT_TEXT} onClose={() => setShowAgreementModal(false)} />
      )}
    </div>
  )
}
